...
extension Dictionary {
    ...
    func getValue(key: Key) throws -> Value {
        if self.keys.contains(key) == true {
            return self[key]!
        }else {
            let error = CustomError(title: "Invalid key ", description: "Key not found \(key) ", code: 10000)
...
extension Double{
    func toCurrency(_ fractionDigits: Int) -> String {
        let str = " ₹ \( self.roundToDecimal(fractionDigits))"
...
extension String{
    ...
    func toCurrency(number : String) -> String {
        return Double(number)?.toCurrency(2) ?? " ₹ 0.00";
    }
