...
class Up_Down_Ftp : NSObject,GRRequestsManagerDelegate{
    ...
    var temp_directory_List = [String]()
    var temp_directory_List2 = [String]()
    var requestsManager : GRRequestsManager!
    var delegate : Up_Down_Output?
    var finalDownloadingList = [String]()
    var isDownaloading = false
    var isDownaloading2 = false
    ...
    var ftp_hostname_download="",ftp_username_download="",ftp_password_download="",ftp_port_download=""
...
//                web_root_path = "/DEMO/Javed1/"
                ...
                ftp_hostname_download = try c[db.getColumnIndex(statement: statement, Coloumn_Name: "ftpip_download")] as! String
                
                ftp_username_download  = try c[db.getColumnIndex(statement: statement, Coloumn_Name: "username_download")] as! String
                // CBO_DOMAIN_SERVER
                
                
                ftp_password_download  = try c[db.getColumnIndex(statement: statement, Coloumn_Name: "password_download")] as! String
                // cbodomain@321
                
                
               ftp_port_download  = try c[db.getColumnIndex(statement: statement, Coloumn_Name: "port_download")] as! String
...
//                  i += 1
                } else if (!file.contains(".") &&  file_Name.contains(file)) {
...
//            print(finalDownloadingList)
            ...
            print("isDownaloading :- ",isDownaloading,"----------------------")
...
//                    currentDir = changeDirectoryUp(path: web_root_path).appending(downloadFromDirectory).appending(directoryName).appending("/")
                    ...
                    currentDir = base_remote_root.appending(directoryName).appending("/")
                    creatSubDirectoryToPhone(root: base_local_root, dirName: directoryName.appending("/"))
...
//                    print(phonePath)
                    ...
                    temp_directory_List = directory_List
                ...
                var temp_directory = downloadFromDirectory.replacingOccurrences(of: base_remote_root, with: "")
                
                temp_directory = temp_directory.substringFrom(offSetTo: (temp_directory.index(of: "/")?.encodedOffset ?? 1) - 1)
                
               
                if temp_directory_List.contains(temp_directory) && !temp_directory_List2.contains(temp_directory){
                    temp_directory_List2.append(temp_directory)
                    main_callCount += child_directory_List.count
                }
...
//                         let currentFile =   changeDirectoryUp(path:currentDir ).appending(saveDirectortTo).appending(fileName)
                         ...
                         let currentFile =   downloadFromDirectory.appending(fileName)
                         remoteFileList.append(currentFile)
                        
                        //saveToPath.append(phonePath.appending(saveDirectortTo).appending(fileName))
                        
                        saveToPath.append(base_local_root.appending(downloadFromDirectory.replacingOccurrences(of: base_remote_root, with: "")).appending(fileName))
...
//                    }
                ...
                for directoryName in child_directory_List{
                    ...
                    creatSubDirectoryToPhone(root: base_local_root.appending(downloadFromDirectory.replacingOccurrences(of: base_remote_root, with: "")), dirName: directoryName.appending("/"))
...
//                    print("5",currentDir)
        ...
        //callCount == temp_child_directory_List.count
        if callCount == main_callCount + temp_directory_List2.count - 1
            && temp_directory_List.count == temp_directory_List2.count
            && childDirFound == true{
          downloadFiles()
        }
...
//        calculatePercant(indexId: Float(indexId ) ,  totalfiles: Float(myArray.count - 1))
    ...
    func downloadSingleFile(file_Name : [String] , item_Name :  [String]) {
        ...
        connnectingwithFTP(ftp_hostname: ftp_hostname_download, ftp_username: ftp_username_download, ftp_password: ftp_password_download)
        
//        print(file_Name , item_Name)
        currentDir = (changeDirectoryUp(path: web_root_path).appending("VisualAid/"))
         
        //base_local_root = getDocumentDirectory()
        base_remote_root = currentDir
        // request to fatch all files from directory
...
//        print(phonePath)
    ...
    func creatSubDirectoryToPhone( root : String, dirName : String)  -> String {
        
            let fileManager = FileManager.default
            
            let phonePath = root.appending(dirName)
