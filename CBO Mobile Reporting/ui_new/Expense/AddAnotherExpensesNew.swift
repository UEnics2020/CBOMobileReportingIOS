//
//  AddAnotherExpensesNew.swift
//  CBO Mobile Reporting
//
//  Created by CBO IOS on 27/11/19.
...
class AddAnotherExpensesNew: CustomUIViewController  , CheckBoxDelegate , FTPUploadDelegate {
      ...
      @IBOutlet weak var rate_txt: CustomTextView!
      ...
      var expexnse:mExpense!
      var otherexp:mOthExpense!
      var eExpenseType:eExpanse!
    ...
    override func viewDidLoad() {
        ...
        setupUI(value: true)
            ...
            //        AttachmentImageView.image = objImagePicker.getPhotoFromCustomAlbum()
                ...
                km_txt.setKeyBoardType(keyBoardType: .numberPad)
                rate_txt.setKeyBoardType(keyBoardType: .numberPad)
                
        
                expexnse = VCIntentArray["expense"] as! mExpense
                otherexp = VCIntentArray["othExpense"] as! mOthExpense
                eExpenseType = VCIntentArray["eExpense"] as! eExpanse
                
                
                if eExpenseType != eExpanse.None {
                    heading.text = "Add \(eExpenseType.name())"
                }
                ...
                if (otherexp.getId() != 0){
                    ...
                    let datanum = DPItem(title: otherexp.getExpHead().getName(),
                                         code: "\(otherexp.getExpHead().getId())",
                                         type : "\(otherexp.getExpHead().getDA_ACTION())");
                    
                    dropDownList.append(datanum);

                }else{
                    dropDownList = cbohelp.get_ExpenseHeadNotAdded(SHOW_IN_TA_DA: eExpenseType);
                   // dropDownList.append(0,DPItem(title:"---Select Exp. Head---",code: "0"));
                }
        
                //dropDownList = cbohelp.get_ExpenseHeadNotAdded()
                ...
                filename = ""
               // if VCIntent["who"] != "0"{
                exp_hed_dp.headerTitle = otherexp.getExpHead().getName()
                exp_id = String(otherexp.getExpHead().getId())
                exp_hed = otherexp.getExpHead().getName()
                    
                   
                    //exp_hed_dp.isUserInteractionEnabled  = false
                    expHead = cbohelp.getEXP_Head(Id: exp_id);
                    
               // }
                
                amt_txt.onTextChanged = { text in
                    let amt = text.isEmpty ? 0 : Double (text)!
                    var maxAmt  = self.expHead?.getMAX_AMT() ?? 0;
                    if (maxAmt == 0) {
                        maxAmt = amt;
                    }
                    if (amt > maxAmt) {
                        
                        AppAlert.getInstance().Alert(vc: self, title: "Alert!!!", massege:
                        "You are not allowed to enter more then \(maxAmt)") {
                            self.amt_txt.setText(text: "\(maxAmt)");
        ...
        //            add_attachment.setChecked(false);
                    ...
                    self.expHead = self.cbohelp.getEXP_Head(Id: self.exp_id);
                    self.amt_txt.setText(text: "");
                    var allreadyAdded = false;
                    if (self.VCIntent["who"] == "0"){
                        if (self.cbohelp.get_ExpenseTypeAdded(typeStr: self.expHead?.getEXP_TYPE_STR() ?? "-1").count > 0
                            && self.expHead?.getEXP_TYPE() ?? eExpanse.None  != eExpanse.None) {
                            allreadyAdded = true;
                     ...
                     if (!allreadyAdded) {
                        ...
                        AppAlert.getInstance().Alert(vc: self, title: "Alert!!!", massege:
                        "\(self.expHead!.getEXP_TYPE()) allready submitted in another Head") {
                            self.ExpHeadDropDown.selectedIndex = 0
                            self.ExpHeadDropDown.headerTitle = self.ExpHeadDropDown.items[0].title
    ...
    extension AddAnotherExpensesNew : ImagePickerDelegate{
