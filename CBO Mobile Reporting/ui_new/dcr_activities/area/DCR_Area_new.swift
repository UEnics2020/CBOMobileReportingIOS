...
import CoreLocation
class DCR_Area_new: CustomUIViewController , CustomPanelDelegate{
    ...
    func onDivertChangeListner(sender: CustomPanel, ischecked: Bool) {
        ...
        case AREA_DILOG:
            ...
            if(ischecked && customVariablesAndMethod.getDataFrom_FMCG_PREFRENCE(vc :context,key : "DIVERT_REMARKYN",defaultValue : "N").uppercased() == ("Y")){
                areaLayout.setDivertRemark(remark: customVariablesAndMethod.getDataFrom_FMCG_PREFRENCE(vc : context,key : "sDivert_Remark",defaultValue : ""));
            }
    ...
    var TP_work_with_name = "", TP_work_with_id = "", TP_area_name = "", TP_area_id = ""
    ...
    func onClickAreaLayout(){
        ...
        if (work_name == "" &&  !work_type_code.contains("_W")
            ...
            &&  !areaLayout.IsDiverted() && !TP_area_name.trimmingCharacters(in: .whitespaces).isEmpty) {
            AppAlert.getInstance().getAlert(vc: context,title: "Alert!!!",massege: "DCR is configured as per TP. To divert please select \"Divert Area\"...");
            
        } else {
            
            var sAllYn = "0", dcr_root_divert = "0";
            if(areaLayout.IsDiverted() ){
                sAllYn="1";
                dcr_root_divert="1";
            }
                        ...
                        "Else Reset your Day Plan from Utility",
                                   listener: { () -> OnClickListener in
                                    class anonymous  : OnClickListener {
                                        var parent : DCR_Area_new!
                                        var sAllYn : String!
                                        
                                        func onPositiveClicked(item: UIView?, result: String) {
                                            parent.openArea(sAllYn : sAllYn,freeze: true);
                                        }
                                        
                                        func onNegativeClicked(item: UIView?, result: String) {
                                            
                                        }
                                        init(parent : DCR_Area_new,sAllYn : String) {
                                            self.parent = parent
                                            self.sAllYn = sAllYn
                                        }
                                    }
                                    return anonymous(parent: self,sAllYn : sAllYn)
                    });
            }  else {
                openArea(sAllYn : sAllYn,freeze: false);
            }
        }
    }
    
     private func openArea( sAllYn : String , freeze : Bool){
    ...
    override func viewDidLoad() {
        ...
        areaLayout.setHint(placeholder: "Press the + sign for Area")
        areaLayout.setDivertRemarkReqd(required: true)
...
//            divert_remark.isHidden = true
            self.areaLayout.setdivertReqd(required: true)
            
        }else{
//            ROUTEDIVERTYN.isHidden = true
//            divert_remark.isHidden = true
            self.areaLayout.setdivertReqd(required: false)
        }
...
//            }
    ...
    func workWithPopulate() {
        ...
        workinkWithDropDown.didSelectedItemIndex = { index in
            ...
            default:
                ...
                self.areaLayout.setDiverted(divert: self.customVariablesAndMethod.getDataFrom_FMCG_PREFRENCE(vc: self, key: "ROUTEDIVERTYN_Checked", defaultValue: "") == "Y")
            ...
            self.areaLayout.setDiverted(divert: self.customVariablesAndMethod.getDataFrom_FMCG_PREFRENCE(vc: self, key: "ROUTEDIVERTYN_Checked", defaultValue: "N") == "Y")
            
            
            if( self.areaLayout.IsDiverted() ){
                self.areaLayout.setDivertRemark(remark: self.customVariablesAndMethod.getDataFrom_FMCG_PREFRENCE(vc : self,key : "sDivert_Remark",defaultValue : ""));
            }
    ...
    private func setUITitles(){
        ...
        if (getDelegate().getDCR().getShowRouteAsPerTP() == ("Y")
            && !areaLayout.IsDiverted()){
            //get_area.setEnabled(false);
            areaLayout.setTitle(text: getDelegate().getDCR().getAreaTitle() + " (As per TP)");
        }else{
...
//    }
    ...
    @objc func onclickSavelistener(_ sender: UIButton) {
        ...
        }else if (areaLayout.IsDiverted() && areaLayout.getDivertRemark().isEmpty ){ //!divert_remark.isHidden && divert_remark.getText().isEmpty) {
    ...
    func forWorking(){
        if (getWorkwith() == "" &&  !work_type_code.contains("_W")
            && getDelegate().getUser().getDesginationID() != ("1")) {
            customVariablesAndMethod.getAlert(vc: context,title: "Alert !!!",msg: "Please Select Work With First .....");
        } else if (getArea() == "") {
            customVariablesAndMethod.getAlert(vc: context,title: "Alert !!!",msg: "Please Select Area First .....");
        } else {
            submitWorking();
        }
    }
    ...
    func submitDCR() {
        work_name = getWorkwith()
        customVariablesAndMethod.setDataInTo_FMCG_PREFRENCE(vc : self, key : "ROUTEDIVERTYN_Checked",value: areaLayout.IsDiverted() ? "Y":"N") ;
    ...
    func submitWorking() {
        ...
        params["sDivert_Remark"] = areaLayout.getDivertRemark()
...
//            customVariablesAndMethod.setDataInTo_FMCG_PREFRENCE(vc :context,key : "TP_root_id",value : TP_root_id);
        ...
        do {
            if(!dataFromAPI.isEmpty){
                ...
                for i in 0 ..< jsonArray.count{
                    ...
                    try customVariablesAndMethod.setDataInTo_FMCG_PREFRENCE(vc: context, key: "DIVERTLOCKYN", value: c.getString(key: "DIVERTLOCKYN") );
                    
                    try customVariablesAndMethod.setDataInTo_FMCG_PREFRENCE(vc: context, key: "FCMHITCALLYN", value: c.getString(key: "FCMHITCALLYN") );
                    ...
                    if (Custom_Variables_And_Method.DCR_ID == "0" ){
                        ...
                        try Alert(title: "Alert !!!",msg: c.getString(key: "MSG") );
                    }else if try ( c.getString(key: "DIVERTLOCKYN") == "Y"){
                         progressHUD.dismiss()
                        try customVariablesAndMethod.setDataInTo_FMCG_PREFRENCE(vc: context,key: "DCR_ID", value: c.getString(key: "DCRID") );
                        try customVariablesAndMethod.setDataInTo_FMCG_PREFRENCE(vc: context, key: "DcrPlanTime_server", value: c.getString(key: "IN_TIME") );
                        try Alert(title: "Alert !!!",msg: c.getString(key: "MSG") );
    ...
    func DownloadAll(){
        if (Custom_Variables_And_Method.DCR_ID != "0") {
            ...
            customVariablesAndMethod.setDataInTo_FMCG_PREFRENCE(vc: context,key: "sDivert_Remark", value: areaLayout.getDivertRemark())
