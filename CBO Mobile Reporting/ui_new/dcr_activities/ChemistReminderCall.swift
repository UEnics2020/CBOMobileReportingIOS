//
//  ChemistReminderCall.swift
//  CBO Mobile Reporting
//
//  Created by CBO IOS on 22/11/19.
...
class ChemistReminderCall: CustomUIViewController {
    ...
    override func viewDidLoad() {
        super.viewDidLoad()
             ...
             che_btn.addTarget(self, action: #selector(OnClickChemLoad), for: .touchUpInside )
             pressedAddChemist.addTarget(self, action: #selector(OnSubmitDr), for: .touchUpInside )
        ...
        func genrateSummary(){
            var headers = [String]()
            var isCollaps = [Bool]() //phdcrchem_rc
            reminder_list = cbohelp.getCallDetail(table: "phdcrdr_rc",look_for_id: "",show_edit_delete: "1")
            do {
                
                summary_list.append([try cbohelp.getMenuNew(menu: "DCR", code: "D_CHEMCALL").getString(key:"D_CHEMCALL") : reminder_list])
            }catch{
                print(error)
            }
    ...
    extension ChemistReminderCall : ParantSummaryAdaptorDalegate{
                ...
                .DecisionAlert(vc: self,
                               ...
                               listener: { () -> OnClickListener in
                                class anonymous  : OnClickListener {
                                    var parent : ChemistReminderCall!
                                    var id: String!
                                    var Dr_name: String!
                                    ...
                                    init(parent : ChemistReminderCall,id: String,Dr_name : String) {
                                        self.parent = parent
                                        self.id = id
        ...
        func getAlert(id  : String  , name : String , title : String){
            ...
            let edit = UIAlertAction(title: title.uppercased(), style: .default) { (action) in
                ...
                self.doc_name = name
                self.dr_id = id
                self.che_name.text = self.doc_name
