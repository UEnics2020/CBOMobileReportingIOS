...
class Doctor_Sample: CustomUIViewController {
    ...
    @IBOutlet weak var selectRemark: CustomBoarder!
    
    @IBOutlet weak var dr_sale: CustomeUIButton!
    ...
    override func viewDidLoad() {
        ...
        if customVariablesAndMethod.getDataFrom_FMCG_PREFRENCE(key: "DCR_DR_REMARKYN", defaultValue: "N") == "Y" {
            selectRemark.isHidden = false
        }
        ...
        Dr_sale_url = customVariablesAndMethod.getDataFrom_FMCG_PREFRENCE(vc: context,key: "DR_SALE_URL",defaultValue: "");
        if (Dr_sale_url.trimmingCharacters(in: .whitespaces).isEmpty){
            dr_sale.isHidden = true
        }
        
        dr_sale.addTarget(self, action: #selector(pressedDrSale), for: .touchUpInside)
    ...
    if(customVariablesAndMethod.getDataFrom_FMCG_PREFRENCE(vc:context,key:"DRCALLPRODUCT_MANDATORYYN",defaultValue: "N") == ("Y")) {
           later.isHidden = true;
        }
    ...
    @objc func pressedDrSale(){
        ...
        if (docList.isEmpty) {
           ...
           customVariablesAndMethod.setDataForWebView(vcself: self, mode: 0, title: doc_name, url: Dr_sale_url+"&DR_ID="+dr_id+"&MONTH="+customVariablesAndMethod.currentDate())
        }
    ...
    @objc func pressedVisualAd(){
        ...
        if ( customVariablesAndMethod.getDataFrom_FMCG_PREFRENCE(key: "DCRSAMPLE_AFTERVISUALAIDYN", defaultValue: "N") == "Y") {
            let vc1 = storyboard.instantiateViewController(withIdentifier: "CheckboxShowVisualAd") as! CheckboxShowVisualAd
            vc1.responseCode = MESSAGE_FROM_CHECK_VISUAL_AD
            vc1.vc =  self
            vc = vc1
        }else{
            vc = storyboard.instantiateViewController(withIdentifier: "ShowVisualAd") as! ShowVisualAd
        }
    ...
    @objc func OnProductLoad(){
        ...
        print(dr_id)
        //visualupdate = cbohelp.getDoctorVisualId()
        print("count ", visualupdate.count)
        if( doc_name == ""){
            customVariablesAndMethod.msgBox(vc: context, msg: "Please select Doctor....")
        } /*else if visualupdate.count == 0{
            customVariablesAndMethod.msgBox(vc: context, msg: "Wrong")
        }*/ else {
            
            print("test \(sample_name) \(sample_pob) \(sample_sample) \(sample_noc) \(sample_name_previous)")
    ...
    @objc func submit(){
        ...
        } else if (Custom_Variables_And_Method.getInstance().isCheckForProduct(s: "D", context: self)) {
    ...
    override func getDataFromApi(response_code: Int, dataFromAPI: [String : NSArray]){
        ...
        case MESSAGE_FROM_CHECK_VISUAL_AD:
            ...
            let ItemIds = cbohelp.getDoctorAllItems(drid: dr_id)
                       
            for i in 0..<selected_group_item_ids.count{
                if (ItemIds.contains(selected_group_item_ids[i])) {
            
                    cbohelp.updatedataforVisualAd(dr_id: dr_id, item: selected_group_item_ids[i], visual: "1");
                } else {
