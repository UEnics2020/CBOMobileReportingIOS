...
class DoctorView: CustomUIViewController {
    ...
    let DOCTOR_REPORT = 2, CHEMIST_REPORT = 3, STOCKIST_REPORT = 4 , REMINDER_REPORT = 5,NLC_REPORT = 6,RX_REPORT = 7
    ...
    override func viewDidLoad() {
        ...
        call_type = VCIntent["call_type"]!
         ...
         if(call_type == "D" || call_type == "R" || call_type == "M" || call_type == "P" || call_type == "DA") {
            ...
            let tables = [0]
            progressHUD.show(text: "Please wait ..." )
            CboServices().customMethodForAllServices(params: params, methodName: "GETDCRRX_VIEW", tables: tables, response_code: RX_REPORT, vc: self)
        ...
        case RX_REPORT:
            if(!dataFromAPI.isEmpty){
                ...
                for i in 0 ..< jsonArray.count{
                    ...
                    nameList.append("\(innerJson["DR_NAME"]!)" )
                    timeList.append(innerJson["IN_TIME"] as! String)
                    sample_name.append("")
                    sample_qty.append("")
                    sample_pob.append("");
                    ...
                    if (innerJson["REMARK"] as! String == ("")) {
                        gift_name.append("\(String(describing: innerJson["QTY_CAPTION"]!)) , \(String(describing: innerJson["AMOUN_CAPTION"]!))");
                        gift_qty.append("\(String(describing: innerJson["QTY"]!)) ,\(String(describing:  innerJson["AMOUNT"]!))");
                    }else{
                        gift_name.append("");
                        gift_qty.append("");
                    }
