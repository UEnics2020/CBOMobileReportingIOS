...
class AdImageViewController: CustomUIViewController  , UIScrollViewDelegate{
    ...
    super.viewDidLoad()
        ...
        let myFilesPath = documentDirectoryPath.appending("/Cbo/Product/\(sample_id[indexId])/")
...
//        changeImageGroup(IndexIdVar: indexId)
    ...
    @objc func openweb(){
        ...
        let myFilesPath1 = documentDirectoryPath1.appending("/Cbo/Product/\(sample_id[indexId])/index.html")
        
        let vc = self.storyboard?.instantiateViewController(withIdentifier: "CBOwebViewViewController") as! CBOwebViewViewController
           
           vc.fileExtention = ".html"
           vc.path = myFilesPath1
           vc.VCIntent["title"] = myTopView.getText()
    ...
    func openWithWebView(){
        ...
        let documentDirectoryPath1 = NSSearchPathForDirectoriesInDomains(.documentDirectory, .userDomainMask, true).first as! String
        let myFilesPath1 = documentDirectoryPath1.appending("/Cbo/Product/\(sample_id[indexId]).mp4")
...
//        print(path)
    ...
    func setImage(){
        ...
        let video_path = documentDirectoryPath1.appending("/Cbo/Product/\(sample_id[indexId]).mp4")
