...
extension DPDropDownMenu {
    ...
    private var originTableFrame: CGRect {
       //frame.size.width  - 50
        return CGRect(x: getframex , y: getframeY + frame.size.height, width: frame.size.width , height: 0)
    }
    ...
    private var itemHeight: CGFloat {
        if items.count > visibleItemCount {
            ...
            return CGFloat(visibleItemCount) * 25//bounds.size.height @rinku
        } else {
            tableView.isScrollEnabled = false
            return CGFloat(items.count) * 25//bounds.size.height @rinku
        }
    }
...
//        var cell = tableView.dequeueReusableCell(withIdentifier: dropDownCellIdentifier) as? DPDropDownMenuCell
        ...
        var tempValues : DPItem = items[indexPath.row]
        cell.item.text = tempValues.title
        cell.item.textColor = AppColorClass.colorPrimaryDark // @rinku
//        if cell == nil {
