...
class Area_DialogViewController: CustomUIViewController, UITableViewDataSource , UITableViewDelegate {
    ...
    @IBAction func pressedDoneButton(_ sender: UIButton) {
        ...
        if (MaxAreaAllowed == 0){
            MaxAreaAllowed = data.count;
        }
        
        if (data.count  <= MaxAreaAllowed ) {
            ...
            AppAlert.getInstance().getAlert(vc: self,title: "Alert!!!",massege: "You are only allowed to select \(MaxAreaAllowed) Area from the list..." );
        }
    ...
    override func viewDidLoad() {
        ...
        myTopView.title.text = msg["header"] as? String
        
        MaxAreaAllowed = Int( msg["max"] as! String) ?? 0;
        freeze = msg["freeze"] as! Bool;
        
        if who == "DCR"{
            let sAllYn = msg["sAllYn"] as? String
            ...
            params["sCompanyFolder"] = cbohelp.getCompanyCode()
            params["iPA_ID"] = "\(Custom_Variables_And_Method.PA_ID)"
            selected_list =  (msg["selected_list"] as! String).components(separatedBy: ",")
             let tables = [0]
    ...
    private func parser_utilities2(dataFromAPI : [String : NSArray]){
        if(!dataFromAPI.isEmpty){
            ...
            objWorkWithAdaptor = Word_With_Adaptor(vc: self , tableView: areaWithTableView, list : list,selected_list: selected_list,freeze : false)
    ...
    private func parser_utilities(dataFromAPI : [String : NSArray])
    {
        do{
            if(!dataFromAPI.isEmpty){
                ...
                for i in 0 ..< jsonArray.count{
                    let innerJson = jsonArray[i] as! [String : AnyObject]
                    list.append(try DCR_Workwith_Model (name: innerJson.getString(key: "AREA"),id: "\(i)"))
                    
                }
                
                
                objWorkWithAdaptor = Word_With_Adaptor(vc: self , tableView: areaWithTableView, list : list,selected_list: selected_list,freeze : freeze)
                areaWithTableView.reloadData()
            }
        }catch {
