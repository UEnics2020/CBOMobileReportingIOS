...
//        let j = 0;
        let seprator = "";
        let visual_id = cbohelp.getDoctorVisualId(drid: dr_id);
...
//                var docitems = cbohelp.getDoctorAllItems();
                ...
                //let actlist = getdoclist();
               // if (actlist.contains(Int(dr_id)!)) {
                    if (visual_id.contains(Mainlist[i].getId())) {
...
//        }
        ...
        if (checkIfPOB_Entered) {
            ...
            customVariablesAndMethod.msgBox(vc: context,msg: "Please promote atleast one product")
        }
    ...
    func getdoclist() -> [Int] {
        var myno = [Int]();
        let doclist = cbohelp.getDoctorList();
