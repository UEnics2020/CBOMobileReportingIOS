...
class Word_With_Adaptor : CheckBoxDelegate{
    func onChackedChangeListner(sender: CheckBox, ischecked: Bool) {
        ...
        case 0:
            ...
            if (freeze && !ischecked && areaList.contains(list[index].getName())){
                AppAlert.getInstance().Alert(vc: vc, title: "Call Found!!!",
                                             massege: "You can't De-select \n" +
                    list[index].getName() +
...
//                    });
    ...
    init(vc : CustomUIViewController,  list : [DCR_Workwith_Model], tableView : UITableView, selected_list : [String], independent_list :  [String]) {
        ...
        for index in 0 ..< list.count{
            if ((selected_list.contains(list[index].getName()) || selected_list.contains(list[index].getId())) && !independent_list.contains(list[index].getName())){
    ...
    init(vc : CustomUIViewController, tableView : UITableView, list : [DCR_Workwith_Model], selected_list : [String] ,freeze :Bool) {
        self.vc = vc
        customVariablesAndMethod = Custom_Variables_And_Method.getInstance()
        ...
        self.freeze = freeze;
        if (freeze){
            areaList =  CBO_DB_Helper.shared.getCalledArea();
        }
    }
...
//        cell?.btnWorkWith.addTarget(self, action: #selector(Word_With_Adaptor.btnWorkWithPressed), for: .touchUpInside)
        ...
        if (freeze && areaList.contains(list[index].getName())){
            list[index].setSelected(selected: true);
        }
