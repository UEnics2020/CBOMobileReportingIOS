...
//
        if (clickCount == 1){
            if (spoIdFromList != ("0")) {
                ...
                vc.VCIntent["CurrencyType"] = context.VCIntent["CurrencyType"]
                context.present(vc, animated: true, completion: nil)
                
               // vc.customVariablesAndMethod1.msgBox(vc: vc,msg: "\(SpoRptAdapter.clickCount)");
            }
            else {
                if( !dataList[position].getSpo_bill_url ().isEmpty){
                    
                    context.customVariablesAndMethod1.setDataForWebView(vcself: context, mode: 0, title: dataList[position].getConsignee(), url: dataList[position].getSpo_bill_url ())
