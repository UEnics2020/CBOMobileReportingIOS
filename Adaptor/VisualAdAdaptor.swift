...
//    view.setBackgroundResource(R.drawable.list_selector_unselected);
    ...
    func tableView(_ tableView: UITableView, didSelectRowAt indexPath: IndexPath) {
       ...
       let VISUALAIDPDFYN = customVariablesAndMethod.getDataFrom_FMCG_PREFRENCE(vc: context , key: "VISUALAIDPDFYN", defaultValue: "N")
        ...
        let myFilesPath1 = documentDirectoryPath1.appending("/Cbo/Product/32.mp4")
          let files = filemanager.enumerator(atPath: myFilesPath1)
        ...
        if VISUALAIDPDFYN == "Y"{
          ...
          let myFilesPath = documentDirectoryPath.appending("/Cbo/Product/CATALOG/Menu.html")
    ...
    func showImagesGallery(index: Int){
        ...
        //let vc = context.storyboard?.instantiateViewController(withIdentifier: "ListAdImageViewController") as! ListAdImageViewController
        vc.indexId = index
    ...
    func openWithWebView(path : String , index : Int){
        ...
        if sample_name[index] == "Catalog" {
            //catalog folder
                let documentDirectoryPath = NSSearchPathForDirectoriesInDomains(.documentDirectory, .userDomainMask, true).first as! String
                paths = documentDirectoryPath.appending("/Cbo/Product/CATALOG/Menu.html")
        }
